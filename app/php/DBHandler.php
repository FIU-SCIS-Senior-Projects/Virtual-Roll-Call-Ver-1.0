...
class DBHandler{
  ...
  function getUser($username){
	...
	function getDocuments(){
		global $db_connection;
		$documents = [];
		$sql = 'SELECT DOCUMENTS.document_ID, DOCUMENTS.Document_Name, DOCUMENTS.Category_ID, DOCUMENTS.Upload_Date, DOCUMENTS.Pinned, DOCUMENTS.Uploaded_By, CATEGORIES.category_name FROM DOCUMENTS INNER JOIN CATEGORIES ON DOCUMENTS.Category_ID = CATEGORIES.Category_ID';
		$stmt = $db_connection->prepare($sql);
		$stmt->execute();
		$stmt->bind_result($id, $name, $catID, $date, $pinned, $uploadedBy, $cat_name);
		while($stmt->fetch()){
			$tmp = ["id" => $id,
			"name" => $name,
			"cat_name" => $cat_name,
	...
	function addDocument($document, $category, $upload_date, $pinned, $uploaded_by){
		global $db_connection;
		$result = ['Added' => false];
		$sql = "INSERT INTO DOCUMENTS (Document_Name, Category_ID, Upload_Date, Pinned, Uploaded_By) VALUES (?,?,?,?,?)"; 
		$stmt = $db_connection->prepare($sql);
		
		if (!$stmt->bind_param('sdsds', $document, $category, $upload_date, $pinned, $uploaded_by))
		{
			echo "Binding parameters failed: (" . $stmt->errno . ") " . $stmt->error;
		}
		if (!$stmt->execute())
